...
import { GridCell } from '../../libs/types/gameTypes';
...
const MapCell = ({ cell, x, y }: Props) => {
    const { currentExplorer, currentExplorerOptions, selectExplorer, explorerMove, addLevelMode, addLevelPositionExplorer, addLevelInfo } = useGameContext();
    const inOptions = () => {
    ...
    const specialStyles = () => {
        ...
        return `${addLevelMode && currentExplorer && 'cursor-pointer'} bg-gray-200`; // 'regular cell'
    }
    const cellClick = () => {
        if (addLevelMode && currentExplorer && !cell && !addLevelInfo.setUpCompleted) {
            addLevelPositionExplorer(x, y);
        }
        if (addLevelMode && !addLevelInfo.setUpCompleted) { return }
        selectExplorer(cell)
        if (inOptions()) {
            explorerMove(x, y)
        }
    ...
    return (
        <td
            ...
            {cell ?
                ...
                :
                <div className={`${inOptions() && 'animate-shake'} flex bg-slate-500 rounded-lg w-1/2 h-1/2 m-auto`}>
